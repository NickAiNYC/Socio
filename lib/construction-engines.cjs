...
function generateQboAuthLink({ contractorId }) {
  return {
    contractorId,
    oneClickAuthUrl: `https://appcenter.intuit.com/connect/oauth2?client_id=SOCIO_QBO&response_type=code&scope=com.intuit.quickbooks.accounting&redirect_uri=https%3A%2F%2Fsocio-one.vercel.app%2Fapi%2Fauth%2Fquickbooks%2Fcallback&state=sec_${contractorId}`,
